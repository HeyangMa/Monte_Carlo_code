...
%--------------------------------------------------------------------------
%parameter setting
function SwendsenWang_Ising_2D_domany
n=3;
Nsite=n*n;
T=(0:0.2:6);
beta=1./T;
...
figure(1);errorbar(T,e,Variance,'bo'); xlabel('T'); ylabel('E'); hold on
figure(2);plot(T,m,'bo');   xlabel('T'); ylabel('M');     hold on
figure(3);plot(T,cv,'bo');  xlabel('T'); ylabel('C_{v}'); hold on
figure(4);plot(T,m2,'bo');  xlabel('T'); ylabel('\chi');  hold on
...
for i=1:n*n
    for j=1:2
        if -ising(i)*ising(nbor(i,j))==1 && rand<1-exp(-beta*(-J2-C))
            bonds(i,j)=1;
        elseif ising(i)*ising(nbor(i,j))==1 && rand<1-exp(-beta*(J2-C))
            bonds(i,j)=1;
        end
    end
end
